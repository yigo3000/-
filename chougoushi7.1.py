...
#7.1 更激进的持仓.只要小于80%仓位，就容易买入。不论初始多少钱，一股只买10000元
#存在问题：
#熊市业绩：2016下半年
#
import talib
...
def handle_bar(context, bar_dict):
    ...
    for i in range(0,total):
        ...
        if ( (cur_position == 0 and ranking_list.iloc[i,1] > BUY_VMA9050) or
            (cur_position == 0 and i<5 and context.portfolio.market_value<0.8*context.portfolio.total_value )#and ranking_list.iloc[i,1]>BUY_VMA9050-2)  # 为了牛市时不空仓
        ):
