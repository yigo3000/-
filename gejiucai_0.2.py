...
#熊市业绩：2016年初-2017中： 累计18.374% 年化11.983%
#
import talib
import numpy as np
...
def handle_bar(context, bar_dict):
    ...
    for stock in context.fundamental_df.columns.values:
        ...
        # 成交量太多或者达到止盈条件
        try:
            if ((long_avg[-1] < 2 * short_avg[-1] or context.portfolio.positions[stock].market_value /
        ...
        # 首次买入条件,应该添加只买业绩好的
        if (cur_position == 0 and long_avg[-1] / short_avg[-1] > 3):
            # 买入
        ...
        # 持仓时买入条件,做T
        try:
            if (cur_position > 0 and context.portfolio.positions[stock].market_value / context.portfolio.positions[
